...
public class test
{
    ...
    /// <param name="mchid ">直连商户号  <para>body直连商户的商户号，由微信支付生成并下发。</para><para>示例值：1230000109</para><para>可 为null</para></param>
    ...
    {
        this.appid = param_name;
        this.mchid = param_name;
        this.description = param_name;
        this.out_trade_no = param_name;
        this.time_expire = param_name;
        this.attach = param_name;
        this.notify_url = param_name;
        this.goods_tag = param_name;
        this.amount = param_name;
        this.payer = param_name;
        this.detail = param_name;
        this.scene_info = param_name;
        this.settle_info = param_name;
    }
    ...
    /// <summary>
    /// 交易结束时间
    /// <para>body 订单失效时间，遵循rfc3339标准格式，格式为YYYY-MM-DDTHH:mm:ss+TIMEZONE，YYYY-MM-DD表示年月日，T出现在字符串中， 表示time元素的开头，HH:mm:ss表示时分秒，TIMEZONE表示时区（+08:00表示东八区时间，领先UTC 8小时，即北京时间）。例如：2015-05-20T13:29:35+08:00表示，北京时间2015年5月20日 13点29分35秒。订单失效时间是针对订单号而言的，由于在请求支付的时候有一个必传参数prepay_id只有两小时 </para>期，所以在重入时间超过2小时的时候需要重新请求下单接口获取新的prepay_id。其他详见时间规则
    /// <para>time_expire只能第一次下单传值，不允许二次修改，二次修改系统将报错。如用户支付失败后，需再次支付，需更换原订单号重新 下单。 </para>
    ...
    /// <summary>
    /// 通知地址
    /// <para>body异步接收微信支付结果通知的回调地址，通知url必须为外网可访问的url，不能携带参数。</para> 
    /// <para>公网域名必须为https，如果是走专线接入，使用专线NAT IP或者私有回调域名 可使用http</para>
    /// <para>可为null</para>
    /// </summary>
    ...
    public class Detail
    {
        ...
        /// 含参构造函数
        /// </summary>
        ///<para>3、该字段主要用于防止同一张小票分多次支付，以享受多次优惠的情况，正常支付订单不必上传此参数。</para><para>示例值：608800</para></param>
        public Detail(int cost_price)
        {
            this.cost_price = param_name;
        }
        ...
        /// <para>                              3、该字段主要用于防止同一张小票分多次支付，以享受多次优惠的情况，正常支付订单不必上传 此参数。 </para>
    ...
    public class Scene_Info
    {
        ...
        {
            this.payer_client_ip = param_name;
        }
    ...
    public class Settle_Info
    {
        ...
        {
            this.profit_sharing = param_name;
        }

public class chapter6_1_1ReturnJson
{
...
/// <param name="total_amount">总金额  <para>1、金额：数字，必须≥0（单位：分）2、总金额=（完结付费项目1…+完结付费项目n）-（完结商户优惠项目1…+完结商户优惠项目n）</para><para>示例值：50000</para></param>
        public chapter6_1_1ReturnJson(string appid, string mchid, string out_order_no, string service_id, string order_id, string service_introduction, string state, string state_description, Post_Payments[] post_payments, Post_Discounts[] post_discounts, Time_Range time_range, Location location, string attach, string notify_url, ulong total_amount)
        {
        this.appid = appid;
this.mchid = mchid;

public class chapter6_1_5ReturnJson
{
...
/// <param name="mchid">商户号 <para>调用接口提交的商户号 （返回用户最近一次授权成功时记录的mchid，不受当前请求参数影响）</para><para>示例值：1230000109</para></param>
/// <param name="openid">用户标识 <para>微信用户在商户对应appid下的唯一标识。  </para><para>示例值：oUpF8uMuAJO_M2pxb1Q9zNjWeS6o</para><para>可为null</para></param>
/// <param name="authorization_code">授权协议号 <para>商户系统内部授权协议号，要求此参数只能由数字、大小写字母_-*组成，且在同一个商户号下唯一。</para><para>示例值：1275342195190894594</para><para>可为null</para></param>
/// <param name="authorization_state">授权状态 <para>标识用户授权服务情况。UNAVAILABLE：用户未授权服务AVAILABLE：用户已授权服务</para><para></para><para>示例值：UNAVAILABLE</para></param>
...
/// <param name="authorization_success_time">最近一次授权成功时间 <para>最近一次授权成功时间，遵循rfc3339标准格式，格式为YYYY-MM-DDTHH:mm:ss.sss+TIMEZONE，YYYY-MM-DD表示年月日，T出现在字符串中，表示time元素的开头，HH:mm:ss.sss表示时分秒毫秒，TIMEZONE表示时区（+08:00表示东八区时间，领先UTC8小时，即北京时间）。例如：2015-05-20T13:29:35.120+08:00表示北京时间2015年05月20日13点29分35秒</para><para>示例值：2015-05-20T13:29:35.120+08:00</para></param>
        public chapter6_1_5ReturnJson(string service_id, string appid, string mchid, string openid, string authorization_code, string authorization_state, string cancel_authorization_time, string authorization_success_time)
        {
...
this.authorization_success_time = authorization_success_time;
 ...
 /// <para>示例值：wxd678efh567hg6787</para>
        ...
        /// <summary>
        /// 商户号
        /// <para>调用接口提交的商户号 （返回用户最近一次授权成功时记录的mchid，不受当前请求参数影响）</para>
 ...
 /// <para>示例值：oUpF8uMuAJO_M2pxb1Q9zNjWeS6o</para>
        ...
        /// <summary>
        /// 授权协议号
        /// <para>商户系统内部授权协议号，要求此参数只能由数字、大小写字母_-*组成，且在同一个商户号下唯一。 </para>
 /// <para>示例值：1275342195190894594</para>
        ...
        /// <summary>
        /// 授权状态
        /// <para>标识用户授权服务情况。 UNAVAILABLE：用户未授权服务 AVAILABLE：用户已授权服务 </para>
    /// <para></para>
